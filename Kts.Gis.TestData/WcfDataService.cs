    ...
    public sealed class WcfDataService : IDataService
    {
        ...
        public WcfDataService(string errorFolderName, string substrateFolderName, string thumbnailFolderName)
        {
            ...
            try
            {
                ...
                bool result = wcfClient1.MobileInitialize("Якутск");
                var getAsll = LoginAccessService.GetAll();
            ...
            */
            ...
            //!!((IClientChannel)wcfClient1).Dispose();
        }

        public bool MobileInitialize(string serverName)
        {
            bool result = false;
            try
            {
                var wcfClient = m_chanelFactory.CreateChannel();
                result = wcfClient.MobileInitialize(serverName);
        ...
        public Task<bool> Initialize(string serverName)
        {
            ...
            {
                var wcfClient = m_chanelFactory.CreateChannel();
                result = wcfClient.Initialize(serverName);
                ((IClientChannel)wcfClient).Close();
            } catch (Exception e)
            {
