...
namespace Kts.Gis.Data
{
    ...
    public sealed partial class SqlFigureAccessService
    {
        ...
        /// <returns>Список объектов.</returns>
        public List<Tuple<string, string>> GetJurObjects(SchemaModel schema, int cityId)
        {
            var suffix = schema.IsIS ? "_is" : "";
            ...
            try
            {
                using (var connection = this.Connector.GetConnection())
                    using (var command = new SqlCommand("get_jur_objects" + suffix, connection))
                    {
                        ...
                        using (var reader = command.ExecuteReader())
                            while (reader.Read())
                                result.Add(new Tuple<string, string>(Convert.ToString(reader["id"]), reader["name"].ToString()));
                    }
            }
            catch (Exception e)
            {
...
and CONVERT(varchar(255),value) = '9A29118C-2E30-4766-B01F-CC6A053A5063'
        ...
        /// <param name="cityId">Идентификатор населенного пункта.</param>
        /// <returns>Список объектов.</returns>
        public List<Tuple<string, string>> GetKvpObjects(SchemaModel schema, int cityId)
        {
            var suffix = schema.IsIS ? "_is" : "";
            ...
            try
            {
                using (var connection = this.Connector.GetConnection())
                    using (var command = new SqlCommand("get_kvp_objects" + suffix, connection))
                    {
                        ...
                        using (var reader = command.ExecuteReader())
                            while (reader.Read())
                                result.Add(new Tuple<string, string>(reader["id"].ToString(), reader["adr"].ToString()));
                    }
            }
            catch (Exception e)
            {
        ...
        public List<Guid> GetIjs(int cityId, SchemaModel schema, int bitFlag)
        {
            var suffix =   "";

            var query = string.Format("get_ijs{0} @city_id = {1}, @year = {2}, @user_id = {3}, @bit_flag = {4}", suffix, cityId, schema.Id, this.loggedUserId, bitFlag);
            ...
            {
                using (var connection = this.Connector.GetConnection())
                using (var command = new SqlCommand("get_ijs" + suffix, connection))
                {
                    command.CommandType = CommandType.StoredProcedure;
                    ...
                    if (!schema.IsIS)
                    {
                        command.Parameters.Add(new SqlParameter("@year", schema.Id));
                        command.Parameters.Add(new SqlParameter("@user_id", this.loggedUserId));
                        command.Parameters.Add(new SqlParameter("@bit_flag", bitFlag));
                    }
                    ...
                    using (var reader = command.ExecuteReader())
                        while (reader.Read())
                        {
                            id = Guid.Parse(reader["id_object"].ToString());
        ...
        /// <param name="schema">Схема.</param>ge
        ...
        /// <param name="schema">Схема.</param>
        public void SetJurId(Guid gisId, string jurId, SchemaModel schema)
        {
            var suffix = schema.IsIS ? "_is" : "";
        ...
        /// <param name="schema">Схема.</param>
        public void SetKvpId(Guid gisId, string kvpId, SchemaModel schema)
        {
            var suffix = schema.IsIS ? "_is" : "";
